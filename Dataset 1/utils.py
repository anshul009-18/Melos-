...
class FreeMusicArchive:
    ...
    def get_all(self, dataset, id_range):
        ...
        # Updated: Use list to collect rows and concat once (performance fix)
        rows_list = []
        ...
        for id_ in id_range:
            try:
                ...
                continue
            row.pop(index)
            # Create a Series with the ID as name to serve as index
            rows_list.append(pd.Series(row, name=id_))

        if rows_list:
            new_rows = pd.concat(rows_list, axis=1).T
...
def load(filepath):
    ...
    if 'tracks' in filename:
        ...
        # Updated: Modern Pandas CategoricalDtype
...
class LibrosaLoader(RawAudioLoader):
    def _load(self, filepath):
        ...
        # Updated: Removed res_type='kaiser_fast' as it is deprecated/removed in new librosa
...
def build_sample_loader(audio_dir, Y, loader):
    class SampleLoader:
        def __init__(self, tids, batch_size=4):
            ...
            # Updated: Fix for ctypes vs numpy string types if necessary
            if isinstance(tids, (pd.Index, pd.Series)):
                 tids = tids.to_numpy()
            
            data = sharedctypes.RawArray(ctypes.c_int, tids)
            self.tids = np.ctypeslib.as_array(data)

            self.batch_size = batch_size
            self.loader = loader
            self.X = np.empty((self.batch_size, *loader.shape))
            self.Y = np.empty((self.batch_size, Y.shape[1]), dtype=int) # Updated: np.int -> int
        ...
        def __next__(self):
            ...
            for tid in tids:
                try:
                    ...
                    self.X[batch_size_actual] = self.loader.load(audio_path)
                    self.Y[batch_size_actual] = Y.loc[tid]
                    batch_size_actual += 1
            ...
            with self.lock2:
                ...
                return self.X[:batch_size_actual], self.Y[:batch_size_actual]
