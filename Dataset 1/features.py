...
def compute_features(tid):
    ...
    # Use context manager for warnings to avoid global suppression if possible
    with warnings.catch_warnings():
        ...
        try:
            ...
            del stft
            # Updated: power_to_db instead of logamplitude
...
def main():
    ...
    nb_workers = max(1, len(os.sched_getaffinity(0)) - 1) if hasattr(os, 'sched_getaffinity') else os.cpu_count()

    table = ((5000, 1), (3000, 3), (2000, 5), (1000, 10), (0, nb_workers))
    for duration, workers in table:
        print('Working with {} processes.'.format(workers))
